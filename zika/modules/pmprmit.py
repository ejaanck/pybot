...
from zika.helpers.SQL.globals import addgvar, gvarstatus
from zika.helpers.tools import get_arg
from config import CMD_HANDLER as cmd
from zika import TEMP_SETTINGS

from .help import add_command_help

DEF_UNAPPROVED_MSG = (
    "•ꜱᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ sɪᴀʀᴀʙ-ᴜsᴇʀʙᴏᴛ•\n"
    "➖➖➖➖➖➖➖➖➖➖\n"
    "ᴊᴀɴɢᴀɴ ᴍᴇʟᴀᴋᴜᴋᴀɴ ꜱᴘᴀᴍ ᴄʜᴀᴛ\n"
    "ꜱᴇʙᴇʟᴜᴍ ᴅɪ ᴋᴏɴꜰɪʀᴍᴀꜱɪ, ᴀᴛᴀᴜ ᴀɴᴅᴀ\n"
    f"ᴀᴋᴀɴ ᴏᴛᴏᴍᴀᴛɪꜱ ᴛᴇʀʙʟᴏᴋɪʀ.\n"
    f"➖➖➖➖➖➖➖➖➖➖\n"
    "ᴍᴀɪɴᴛᴇɴᴀɴᴄᴇ ʙʏ☕ @Dhilnihnge"
)
...
async def incomingpm(client: Client, message: Message):
    try:
        from zika.helpers.SQL.globals import gvarstatus
        from zika.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from zika.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
...
async def approvepm(client: Client, message: Message):
    try:
        from zika.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from zika.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from zika.helpers.SQL.globals import addgvar
