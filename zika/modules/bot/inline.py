...
from zika.helpers.data import Data
from zika.helpers.inline import inline_wrapper, paginate_help
...
async def ping_function(message: Message, answers):
    ...
    msg = (
        ...
        f"      Bᴏᴛ › <code>{BOT_VER}</code> \n"
        f"      Pʏᴛʜᴏɴ ›</b> <code>{pyver.split()[0]}</code> \n"
        f"      Pʏʀᴏɢʀᴀᴍ ›</b> <code>{pyrover}</code> \n"
    )
    answers.append(
        InlineQueryResultArticle(
            title="arab",
